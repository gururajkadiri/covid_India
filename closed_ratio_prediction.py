...
arr=load_Data(start_day=start_day,day_max=56,state="India")
x_pts=column(arr,0)
[x0_cases, r_cases,delta_cases], pcov  = optimize.curve_fit(exp_growth_guru, x_pts, column(arr,1), p0=(0, 0.1,column(arr,1)[0]),method="lm")
[x0_recovered, r_recovered,delta_recovered], pcov  = optimize.curve_fit(exp_growth_guru, x_pts, column(arr,2), p0=(0, 0.1,column(arr,2)[0]),method="lm")
[x0_dead, r_dead,delta_dead], pcov  = optimize.curve_fit(exp_growth_guru, x_pts, column(arr,3), p0=(0,0.1,column(arr,3)[0]),method="lm")
